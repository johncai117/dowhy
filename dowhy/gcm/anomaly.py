...
def estimate_conditional_anomaly_score(parent_samples: np.ndarray,
                                       target_samples: np.ndarray,
                                       causal_model: ConditionalStochasticModel,
    ...
    for i in range(parent_samples.shape[0]):
        samples_from_conditional = causal_model.draw_samples(repmat(parent_samples[i], num_samples_conditional, 1))
...
def estimate_anomaly_scores(causal_model: ProbabilisticCausalModel,
    ...
    for node in tqdm(causal_model.graph.nodes, desc='Estimating conditional anomaly scores', position=0, leave=True,
        ...
        if is_root_node(causal_model.graph, node):
            ...
            results[node] = estimate_conditional_anomaly_score(tmp_anomaly_parent_samples,
...
def attribute_anomaly_scores(causal_model: InvertibleStructuralCausalModel,
                             target_node: Any,
    ...
    attributions = estimate_anomaly_score_attributions(noise_of_anomaly_samples[nodes_order].to_numpy(),
...
def estimate_anomaly_score_attributions(samples_of_interest: np.ndarray,
    ...
    """
    Estimates the contributions of the features for each sample in samples_of_interest to the anomaly score obtained by
    the anomaly_scoring_func. If attribute_mean_deviation is set to False, the anomaly score is based on the information
    theoretic  (IT) score -log(P(g(X) >= g(x))), where g is the anomaly_scoring_func, X samples from the marginal
    distribution of the target_node and x an observation of the target_node in anomaly_samples. If
    ...
    :param samples_of_interest: Samples x for which the contributions are estimated.
