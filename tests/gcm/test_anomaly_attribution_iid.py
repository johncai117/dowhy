...
from dowhy.gcm import auto, MedianCDFQuantileScorer, InverseDensityScorer, fit, \
    AdditiveNoiseModel, PredictionModel, attribute_anomaly_scores, InvertibleStructuralCausalModel
from dowhy.gcm.anomaly import estimate_anomaly_score_attributions, _relative_frequency
...
def test_given_simple_gaussian_data_when_estimate_anomaly_score_attributions_with_it_score_then_returns_qualitatively_correct_results():
    ...
    contributions = estimate_anomaly_score_attributions(anomaly_samples,
...
def test_given_simple_causal_chain_with_linear_relationships_when_attribute_anomaly_scores_with_it_score_then_returns_qualitatively_correct_results():
    ...
    scores = attribute_anomaly_scores(causal_model,
                                      'X3',
                                      anomaly_data,
    ...
    assert scores['X0'][0] + scores['X1'][0] + scores['X2'][0] + scores['X3'][0] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[0]), abs=0.001)
    assert scores['X0'][1] + scores['X1'][1] + scores['X2'][1] + scores['X3'][1] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[1]), abs=0.001)
    assert scores['X0'][2] + scores['X1'][2] + scores['X2'][2] + scores['X3'][2] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[2]), abs=0.001)
...
def test_given_non_trivial_graph_with_nonlinear_relationships_when_attribute_anomaly_scores_with_it_score_then_returns_qualitatively_correct_results():
    ...
    X3 = MyNonLinearModel().predict(np.column_stack([X1, X2])) \
         + np.random.normal(0, 1, (num_training_samples, 1))  # (X1 + X2)**2 + N
    ...
    # The sum of the scores should add up to the anomaly score of the target (here, X3).
    assert scores['X0'][0] + scores['X1'][0] + scores['X2'][0] + scores['X3'][0] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[0]), abs=0.4)
    assert scores['X0'][1] + scores['X1'][1] + scores['X2'][1] + scores['X3'][1] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[1]), abs=0.4)
    assert scores['X0'][2] + scores['X1'][2] + scores['X2'][2] + scores['X3'][2] \
           == approx(total_anomaly_score(training_data, anomaly_data['X3'].to_numpy()[2]), abs=0.6)
