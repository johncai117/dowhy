...
from dowhy.gcm import auto, AdditiveNoiseModel, fit, estimate_anomaly_scores, \
...
from dowhy.gcm.anomaly import estimate_conditional_anomaly_score
...
def test_estimate_anomaly_scores_using_median_cdf_quantile_anomaly_scoring():
    ...
    assert max(estimate_anomaly_scores(causal_model,
...
def test_estimate_anomaly_scores_does_not_raise_an_error_with_multivariate_inputs():
    ...
    anomaly_scores = estimate_anomaly_scores(causal_model, data_anomaly)
    assert anomaly_scores['X1'][0] == approx(-np.log(EPS), abs=3)
...
def test_estimate_conditional_anomaly_score():
    ...
    assert estimate_conditional_anomaly_score(X[:5], Y[:5], causal_model, MeanDeviationScorer).reshape(-1) \
